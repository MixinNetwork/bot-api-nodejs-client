...
export const MutilsigsKeystoreClient = (axiosInstance: AxiosInstance, keystore: Keystore | undefined) => {
  const initMutilsig = (pin: string, request_id: string, action: MultisigAction): Promise<MultisigRequestResponse> => {
    ...
    return axiosInstance.post<unknown, MultisigRequestResponse>(`/multisigs/requests/${request_id}/${action}`, { pin: encrypted });
  };

  return {
    // Get signature outputs, if an account participates in it
    index: (params: MultisigIndexRequest): Promise<MultisigUTXOResponse[]> => {
    ...
    sign: (pin: string, request_id: string): Promise<MultisigRequestResponse> => initMutilsig(pin, request_id, 'sign'),

    // Cancel multisigs
    unlock: (pin: string, request_id: string): Promise<MultisigRequestResponse> => initMutilsig(pin, request_id, 'unlock'),

    // Cancel my signature
    cancel: (pin: string, request_id: string): Promise<MultisigRequestResponse> => initMutilsig(pin, request_id, 'cancel'),
  };
};
