...
class Auth {
  ...
  static signAuthenticationToken(methodRaw: string | undefined, uri: string, params: Object | string, keystore: Keystore | undefined) {
    if (!keystore) {
      return '';
    }
    ...
    const sign = base64.RawURLEncode(signData);
    result.push(sign);
    return result.join('.');
  }
