...
const toBytes = (data: Buffer) => new Uint8Array(data);
...
export const newHash = (data: Buffer) => Buffer.from(sha3_256.create().update(toBytes(data)).digest());

export const sha256Hash = (data: Buffer) => Buffer.from(sha256.create().update(toBytes(data)).digest());

export const sha512Hash = (data: Buffer) => Buffer.from(sha512.create().update(toBytes(data)).digest());

export const blake3Hash = (data: Buffer) => Buffer.from(blake3.create({}).update(toBytes(data)).digest());
