...
const isReduced = (x: Buffer) => {
  for (let i = x.byteLength - 1; i >= 0; i--) {
    ...
    else if (x.at(i)! < scMinusOne.at(i)!) return true;
  }
  return true
};
...
const sign = (msg: Buffer, key: Buffer) => {
  ...
  const messageDigest = Buffer.from(sha512Hash(Buffer.concat([
    digest1.subarray(32),
    msg
  ])), 'hex');
  ...
  const hramDigest = Buffer.from(sha512Hash(Buffer.concat([
    r,
    pub,
    msg
  ])), 'hex');
