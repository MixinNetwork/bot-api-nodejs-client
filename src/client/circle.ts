...
export const CircleKeystoreClient = (axiosInstance: AxiosInstance) => ({
  ...
  addConversation: (conversationID: string, circleID: string): Promise<CircleResponse[]> => axiosInstance.post<unknown, CircleResponse[]>(`/conversations/${conversationID}/circles`, { circleID, action: 'ADD' }),

  // Remove the group from a certain circle
  removeConversation: (conversation_id: string, circleID: string): Promise<CircleResponse[]> => axiosInstance.post<unknown, CircleResponse[]>(`/conversations/${conversation_id}/circles`, { circleID, action: 'REMOVE' }),
});
